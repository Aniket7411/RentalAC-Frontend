...
const ServiceBookingModal = ({ service, isOpen, onClose, onSubmit, initialData }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Payment option removed - will be selected at checkout
  });
  ...
  const validateStep = (stepNumber) => {
    ...
    // Step 3 removed - payment option will be selected at checkout
  ...
  const handleNext = () => {
    if (validateStep(step)) {
      if (step === 2) {
        // Skip to review (step 3, previously step 4)
        setStep(3);
      } else {
        setStep(step + 1);
      }
  ...
  const handleSubmit = async () => {
    // Only validate steps 1 and 2 (booking details)
    ...
    try {
      ...
      const bookingDetails = {
        ...
        date: formData.date, // Alias for compatibility
        time: formData.time, // Alias for compatibility
        // Payment option will be selected at checkout
      };

      // Check if this is edit mode (has initialData and onSubmit handler)
                  ...
                  <div key={s} className="flex items-center flex-1">
                    <div className="flex flex-col items-center flex-1">
                      ...
                      <span className="text-xs mt-1 text-slate-500">
                        {s === 1 ? 'Date & Time' : s === 2 ? 'Address' : 'Review'}
                      </span>
                    </div>
                    {s < 3 && (
                      <div
