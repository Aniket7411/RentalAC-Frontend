...
const ACCard = ({ ac }) => {
  ...
  return (
    ...
    >
      <Link to={`/ac/${ac._id || ac.id}`} className="relative block h-64 sm:h-72 md:h-80 lg:h-96 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden cursor-pointer">
      ...
      <div className="p-4 sm:p-5 md:p-6 flex-1 flex flex-col">
        <h3 className="font-bold text-base sm:text-lg md:text-xl text-text-dark mb-2 line-clamp-2">{ac.brand} {ac.model}</h3>
        <p className="text-sm sm:text-base text-text-light mb-3 font-medium">{ac.capacity} • {ac.type}</p>

        {/* Per Month Price (calculated from 11 months) */}
        <div className="mb-4">
          <div className="flex items-baseline space-x-1">
            <span className="text-2xl sm:text-3xl md:text-4xl font-bold text-primary-blue">
              ₹{calculatePerMonthPrice().toLocaleString()}
            </span>
            <span className="text-sm sm:text-base text-text-light">/month</span>
          </div>
